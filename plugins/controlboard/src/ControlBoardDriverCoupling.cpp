...
{
    if (m_coupledJoints.size()!=m_couplingSize) return false;
    current_vel[m_coupledJoints[0]] = -current_vel[m_coupledJoints[0]]*3;
    return true;
}
...
bool FingersAbductionCouplingHandler::decoupleAcc (yarp::sig::Vector& current_acc)
{
    ...
    if (m_coupledJoints.size()!=m_couplingSize) return false;
    current_acc[m_coupledJoints[0]] = -current_acc[m_coupledJoints[0]]*3;
    return true;
}
...
{
    if (m_coupledJoints.size() != m_couplingSize) return false;
    pos_limits[m_coupledJoints[0]].min = 0.0;
